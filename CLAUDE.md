...
- `getModels()` — returns `Map<label, definition>` where definition flags capabilities: `tools`, `structured_output`, `audio`, `image_generation`, `reasoning` (the OpenAI Responses adapter only sends `include: ['reasoning.encrypted_content']` + `reasoning: {summary}` when set — non-reasoning models like `gpt-4o` reject them), `tokens` (context window), `tiktoken` (encoding name).
- `generate(model, thread, tools, options)` — **async generator** (Phase 1 of v3 refactor). Yields streaming deltas during generation and `return`s the final assembled `Message[]`. Delta union: `{type: 'text_delta', content}`, `{type: 'reasoning_delta', content}`, `{type: 'tool_call', content: {id?, name, arguments}}` (emitted complete), `{type: 'image', content, meta}`, `{type: 'usage', content: {input, output, cached, context}}` (once at end-of-stream, via `Model.usageDelta()`, only when the provider reported usage — `input` includes `cached`; `context` = `input + output`; Anthropic's cache read/creation tokens are folded back into `input` since the API reports them outside `input_tokens`). `Agent.generateCompletion()` (Phase 2) is itself an async generator: it forwards `text_delta` to consumers as `{type:'chunk', content}` events, forwards `usage` as `{type:'usage', content}` (also stamping `thread.state.usage`, last call wins), and returns the assembled `Message[]`. Other delta types are not forwarded yet and only contribute to the final assembly. Tool-call deltas from chat-completions-style APIs (OpenAI legacy, Groq) are accumulated per `index` and yielded once at end-of-stream; on those APIs the usage chunk arrives last with empty `choices` (OpenAI/DeepSeek need `stream_options: {include_usage: true}`, sent by `LegacyOpenAIModel`; Groq ships it natively under `x_groq.usage`).
...
| `{type:'usage', content}` | `{input, output, cached, context}` | Provider-reported token usage, one per LLM call (so several per turn when tools run). `context` = `input + output` — tokens occupying the model context after the call. Mirrored on `thread.state.usage` (last call wins, persisted with the thread). Absent when the provider reports no usage. |
